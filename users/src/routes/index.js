...
router.get("/api/user/enrollmentdata", auth, async function (req, res, next) {
	try {
		let data = await fs.readFileSync(path.resolve("enrollmentMetadata.json"))
		res.send(data)
...
router.get("/api/course/get", auth, async function (req, res, next) {
	...
	try {
		...
		if (type === "course") {
			data = await fs.readFileSync(path.resolve("courses.json"))
			
		} else if (type === "program") {
			data = await fs.readFileSync(path.resolve("program.json"))
		} else if (type === "professional-service") {
			data = await fs.readFileSync(path.resolve("services.json"))
		}
